...
def cast_value(
    ...
    if not sizes:
        try:
            ...
            raise ValueError(
                f"Failed to cast {name!r} as {class_}, got {value}"
            ) from error
    else:
        try:
            arr = numpy.array(value, dtype=class_)
            if arr.dtype == numpy.object_:
                arr = numpy.vectorize(class_, otypes=[numpy.object_])(arr)
        except Exception as error:
            raise ValueError(
                f"Failed to construct numpy.ndarray(dtype={class_!r}) "
                f"from {name!r}, got {value}"
            ) from error

        if arr.ndim != len(sizes):
            raise ValueError(
                f"Array {name!r} dimensions "
                f"must be {len(sizes)}, got {arr.ndim}"
            )
        for idim, (expected_size, actual_size) in enumerate(
            zip(sizes, arr.shape)
        ):
            ...
            if expected_size != actual_size:
                raise ValueError(
                    f"Array {name!r} size of dimension #{idim} "
                    f"must be {expected_size}, got {actual_size}"
                )
